...
| 2 | — | power-sector regulation (S1–S4) | — | **PARTIAL** (2026-08-04): the curated scenario surface (`src/define_uk/scenarios.py`) is gated against the pinned oracle run by `tests/test_scenarios.py` — finite deltas, cross-block baseline consistency, sign sanity, and 11 pinned delta points so silent changes fail loudly. Comparison against the published scenario figures remains **pending**: define-model.org and its DEFINE-UK page state no machine-readable scenario numbers (the paper's results are figure-only), so no published-figure tolerance can be recorded yet. |
| 3 | — | green public investment (S8–S9) | — | **PARTIAL** (2026-08-04), same basis as target 2. Grounded signs from the cached run (annual-mean deltas vs the same folder's baseline): green public investment DEPRESSES real GDP slightly in early years (−1.125 in 2026) before turning strongly positive (+4.08 in 2030, +7.75 in 2035) while cutting emissions (−12.28 in 2030, −13.54 MtCO2e/yr in 2035); housing regulation is contractionary for GDP throughout while cutting emissions; the mixed policy delivers the largest 2035 emissions cut (−23.10). Published-figure comparison **pending** as above. |
